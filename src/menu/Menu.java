...
public class Menu {
	...
	public static void main(String[] args) {
		...
		if (args.length > 0) {
			if (args[0] == "-name") {
				...
				calculer(args);
			} else if (args[0] == "-rejoue") {
				rejouer(args);
			} else if (args[0] == "-simul") {
				simuler(args);
			} else {
				System.err.println("Argument(s) non reconnu(s).");
			}
		} else {
			System.err.println("Argument(s) manquant(s), entrée l'argument -h pour optenir de l'aide.");
		}
	...
	public static void jouer(String[] args) {
		...
		} else if (args.length == 4 && args[2] == "-niveau.") {
			int niveau = -1;
			try {
				niveau = Integer.valueOf(args[3]);
			} catch (Exception e) {
				System.err.println("Le 4eme argument doit être un entier.");
			}
			if (niveau > -1) {
	...
	public static void calculer(String[] args) {
		if (args.length == 5) {
			...
			try {
				...
				System.err.println("Le 5eme argument doit être un entier.");
			}
			if (niveau > -1) {
