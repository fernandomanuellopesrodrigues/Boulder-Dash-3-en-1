...
public class ScorePanel extends JPanel {
	...
	protected void paintComponent(Graphics g) {
		...
		g.drawString("Diamants : " + Partie.gererNiveau.getNbDiamants() + "/"
				+ Partie.gererNiveau.getNiveau().getDiamonds_required(), getWidth() / 15, getHeight() / 2);
		g.drawString("Score : " + Partie.gererNiveau.getScore(), getWidth() * 19 / 70, getHeight() / 2);
		g.drawString("Temps restant : " + Partie.gererNiveau.getTempsRestant(), getWidth() * 45 / 100, getHeight() / 2);
		g.drawString("Niveau : " + Partie.niveau + "/" + Partie.ensembleDeNiveau.getNombre_de_niveaux(),
				getWidth() * 2 / 3, getHeight() / 2);
		g.drawString("Points/diamant : " + (Partie.gererNiveau.getNiveau().getDiamond_value()), (getWidth() * 42) / 50,
				getHeight() * 12 / 30);
